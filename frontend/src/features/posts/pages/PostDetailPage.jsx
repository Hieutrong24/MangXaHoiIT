// src/features/posts/pages/PostDetailPage.jsx
...
function filenameFromUrl(url) {
  try {
    ...
    const last = String(url || "").split("/").filter(Boolean).pop() || "file";
    return last;
  }
}
        ...
        {showDebug && (
          <div className="mt-4 rounded-xl border border-white/10 bg-black/30 p-4 text-xs text-slate-200/90">
            ...
            <div className="grid gap-1">
              ...
              <div>
                <span className="text-slate-400">others:</span> {others.length}
              </div>
              {meta?.correlationId ? (
                <div>
        ...
        {others.length > 0 && (
          <div className="mt-6 rounded-2xl border border-white/10 bg-white/5 p-4">
            <div className="flex items-center gap-2 text-sm font-bold text-slate-100">
              ...
              Tệp đính kèm
              <span className="text-slate-400 font-normal">({others.length})</span>
            </div>
          ...
          <div className="mt-4 space-y-3">
            {post.comments.map((c, idx) => (
              ...
              >
                ...
                <div className="mt-2 text-sm text-slate-200/90">{c.text}</div>
              </div>
            ))}
          </div>
        ) : (
          <div className="mt-3 text-sm text-slate-400">Chưa có bình luận.</div>
        )}
      </GlassCard>
    </div>
  );
