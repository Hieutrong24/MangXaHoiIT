// src/features/feed/pages/HomeFeedPage.jsx
...
export default function HomeFeedPage() {
  ...
  const filteredPosts = useMemo(() => {
    ...
    if (tagMap.has(activeFilter)) {
      list = list.filter(
        (p) => String(p.tag || "").toLowerCase() === activeFilter.toLowerCase()
      );
    }
    ...
    if (query.trim()) {
      ...
      list = list.filter((p) => {
        ...
        const g = String(p.tag || "").toLowerCase();
                ...
                it.type === "post" ? (
                  <PostCard
                    ...
                    onToggleLike={(post, next) => {
                      feedApi.toggleLike?.(post?._id || post?.id, next);
                    }}
                  />
                ) : (
                  <JobCard
